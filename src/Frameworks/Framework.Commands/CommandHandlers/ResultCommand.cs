...
namespace Framework.Commands.CommandHandlers
{
    public class ResultCommand
    {
        ...
        public static implicit operator ResultCommand(ContentResult result) => new(true, ResultCode.Success, result.Content);
        ...
        public static implicit operator ResultCommand(ResponseCommand? data)
        {
                return data.Status switch
                {
                    ResultStatus.Success => new ResultCommand(true, ResultCode.Success,
                        data.Data),
                    ResultStatus.Duplicate => new ResultCommand(false, ResultCode.Duplicate,
                        null,ResultCode.Duplicate.ToDisplay(DisplayProperty.Name)),
                    ResultStatus.NotFound => new ResultCommand(false, ResultCode.NotFound,
                        null,ResultCode.Duplicate.ToDisplay(DisplayProperty.Name)),
